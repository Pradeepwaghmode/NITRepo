...
//	}
	...
	@Override
	/*
	 * public Movie featchMoviById(Integer mid) { // TODO Auto-generated method stub
	 * Optional<Movie> opt=repo.findById(mid); if(opt.isPresent()) return opt.get();
	 * else throw new IllegalArgumentException("Rocord not found !!");
	 * 
	 * }
	 */

	/*
	 * public Movie featchMoviById(Integer mid) { return
	 * repo.findById(mid).orElseThrow(()->new
	...
	public String deleteById(List<Integer> list) {
		...
		Iterable<Movie> movies = repo.findAllById(list);

		
			if (((List<Movie>) movies).size() != 0 && list.size() == ((List<Movie>) movies).size()) {
				repo.deleteAllById(list);
				return count + " Record Deleted !! ";
			
		}
		return "some of record is not in our db to delete";
	}
